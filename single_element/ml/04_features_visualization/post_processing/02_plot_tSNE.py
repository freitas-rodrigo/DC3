...
materials = ['Al', 'Fe', 'Ti', 'Si', 'H$_2$O', 'NaCl']
...
################################################################################
  ...
  for i in range(n_lattices):
    ax.plot(X[y==y_lattices[i],0], X[y==y_lattices[i],1], 'C%d%s' % (i,markers[i]), alpha=0.15, mew=0, zorder=9-i, label=lattices[i].upper())
    ax.plot(X0[i,0], X0[i,1], 'k%s' % markers[i], zorder=100)
  ...
  # Save figure.
  fig.savefig("figures/fig_tSNE_synthetic_crystal_perplexity_%d.pdf" % perplexity)
  plt.close()
...
################################################################################
  ...
  # Plot.
  for i in range(n_lattices):
    ax.plot(X[y==y_lattices[i],0], X[y==y_lattices[i],1], 'C%d%s' % (i,markers[i]), alpha=0.15, mew=0, zorder=9-i, label="%s (%s)" % (materials[i],lattices[i].upper()))
    ax.plot(X0[i,0], X0[i,1], 'k%s' % markers[i], zorder=100)
  ...
  # Save figure.
  fig.savefig("figures/fig_tSNE_md_crystal_perplexity_%d.pdf" % perplexity)
  plt.close()
