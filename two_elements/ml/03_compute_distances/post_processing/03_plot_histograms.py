...
for i in range(len(md_lattices)):
  ...
  ax.plot(x, rho, 'C7-', label='Molecular Dynamics', lw=3)
  ...
  # Add details.
  ax.set_xlabel(r'Distance $\delta_{y_i}(\mathbf{x}_i)$', fontsize=18)
  ax.set_ylabel(r'Density distribution', fontsize=18)
  ax.set_xlim(0,60)
  ax.set_ylim(0)
  lg = ax.legend(fontsize=12, title='%s structure'%lattices[arg], framealpha=1)
  lg.get_title().set_fontsize(12)
  lg.get_title().set_fontweight('bold')
  ax.tick_params(axis='both', which='major', labelsize=12)
  
  # Save figure.
  fig.savefig("figures/fig_distance_histogram_%s.png" % md_lattices[i], dpi=400)
  plt.close()
