...
################################################################################
  ...
  for i in range(n_lattices):
    ax.plot(X[y==y_lattices[i],0][:N], X[y==y_lattices[i],1][:N], 'C%d%s' % (i,markers[i]), alpha=0.05, mew=0, zorder=9-i, label=lattices[i].upper())
  ...
  for l in legend.legendHandles:
    l.set_alpha(1) # Set marker alpha to opaque.

  
  # Save figure.
  fig.savefig("figures/fig_PCA_%s.pdf" % system)
  plt.close()
