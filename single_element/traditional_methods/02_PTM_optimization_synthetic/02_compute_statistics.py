...
lattice_to_y = {'other':0, 'fcc':1, 'hcp':2, 'bcc':3, 'sc':5, 'cd':6, 'hd':7}
...
################################################################################
# Compute minimum accuracy for each liquid data set.                           #
################################################################################

y = loadtxt('data/y/y_liquid.dat')
min_acc_liquid = sum(y==lattice_to_y['other']) / len(y)
...
RMSD = loadtxt('data/RMSD/RMSD_liquid.dat')
y = loadtxt('data/y/y_liquid.dat')
RMSD = RMSD[y!=lattice_to_y['other']]
h, x = histogram(RMSD,bins=200,range=[0,0.6],density=True)  
x = x[:-1] + (x[1]-x[0])/2
cumm_distribution = cumtrapz(h,x,initial=0)
accuracy = min_acc_liquid + (1-cumm_distribution) * (1-min_acc_liquid)
savetxt('data/statistics/liquid.dat', column_stack((x,h,accuracy)), fmt='%.10f %13.10f %.10f', header='RMSD | histogram | PTM accuracy')
...
acc_liquid = loadtxt('data/statistics/liquid.dat', unpack=True, usecols=[2])
for n in range(len(lattices)):
