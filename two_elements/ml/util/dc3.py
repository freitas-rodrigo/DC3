...
def compute_distance(X,y):
  ...
  y0 = read('../02_organize_data/data/perfect_lattices/y.dat')
  d = zeros(len(y))
  for i in range(len(y_lattices)):
    mask = (y==y_lattices[i])
    args = argwhere(y0==y_lattices[i])
    d[mask] = minimum(norm(X[mask]-X0[args[0]],axis=1),norm(X[mask]-X0[args[1]],axis=1))
  return d
